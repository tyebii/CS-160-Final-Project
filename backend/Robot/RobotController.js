...
const getFaultyRobot = (req, res) => {
  ...
  pool.query(sqlQuery, (err, results) => {
    ...
    //modified by Austin now returns two keys, "count" returns number of faulty robots to be used for # of triggers, and "robots" maintains original functionality of query
    res.status(200).json({
...
const updateRobot = (req, res) => {
  ...
  const sqlQuery =
    "Update robot set CurrentLoad = ?, RobotAddress = ?, RobotStatus = ?, Maintanence = ?, Speed = ?, BatteryLife = ?, EstimatedDelivery = ? Where RobotID = ?";
  pool.query(
    sqlQuery,
    [
      CurrentLoad,
